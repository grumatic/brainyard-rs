...
usage() {
  cat <<'USAGE'
Usage: capture-clojure-cli-snapshots [--bin PATH] [--out DIR] [--home DIR] [--keep-home]
...
if [[ -z "$bin_path" ]]; then
  if [[ -x "$repo_root/projects/agent-tui-app/target/by" ]]; then
    bin_path="$repo_root/projects/agent-tui-app/target/by"
  elif command -v by >/dev/null 2>&1; then
    bin_path="$(command -v by)"
  else
    echo "Could not find Clojure by binary. Pass --bin PATH or set BY_CLOJURE_BIN." >&2
    exit 2
  fi
fi
...
capture_case() {
  ...
  NO_COLOR=1 \
    "$bin_path" "$@" >"$stdout_file" 2>"$stderr_file" || status=$?
  printf '%s\n' "$status" >"$status_file"
}
